...
class CaptionTSVDataset(Dataset):
    def __init__(self, data_type, tokenizer, args, is_train):
        ...
        with open('/home/hdd1/vibhav/py-bottom-up-attention/demo/data/genome/1600-400-20/objects_vocab.txt') as f:
    ...
    def get_od_labels(self, idx):
        obj_labels = self.data['image_obj_lbl_ids'][idx]
        obj_labels = [self.vg_classes[i] for i in obj_labels]
        return " ".join(list(set(obj_labels)))

    def get_image_features(self, idx):
        feat = self.data['image_feat'][idx]
        bbox = self.data['image_bbox'][idx]
        pos_vec = torch.stack(
