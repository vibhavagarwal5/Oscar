...
class BertForImageCaptioning_SeqClf(CaptionPreTrainedModel, BertPreTrainedModel):
    """
    Bert for Image Captioning and sequence classification (both heads)
    """

    def __init__(self, config):
        super(BertForImageCaptioning_SeqClf, self).__init__(config)
        self.config = config
        self.bert = BertImgModel(config)
        self.transform = BertPredictionHeadTransform(config)
        bert_embedding_weight = self.bert.embeddings.word_embeddings.weight
        self.decoder = nn.Linear(bert_embedding_weight.size(1),
                                 bert_embedding_weight.size(0), bias=False)
        self.mask_loss = nn.CrossEntropyLoss(reduction='mean')
        self.drop_worst_ratio = 0.2
        # ----------------------------------------------------------------
        self.num_labels = config.num_labels
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
    ...
    def encode_forward(self, input_ids, img_feats, attention_mask, masked_pos, masked_ids=None, labels=None, token_type_ids=None, position_ids=None, head_mask=None, encoder_history_states=None):
        bert_outputs = self.bert(input_ids, img_feats=img_feats, attention_mask=attention_mask,
                                 position_ids=position_ids, token_type_ids=token_type_ids,
                                 head_mask=head_mask,
                                 encoder_history_states=encoder_history_states)
        sequence_output = bert_outputs[0][:, :masked_pos.shape[-1], :]
        ...
        if masked_ids is not None:
            ...
            sequence_output_masked = sequence_output[masked_pos == 1, :]
            transformed_output_masked = self.transform(sequence_output_masked)
            mask_logits = self.decoder(transformed_output_masked)
            masked_ids = masked_ids[masked_ids != 0]   # remove padding masks
            masked_loss = self.mask_loss(mask_logits.float(), masked_ids)
            outputs = (masked_loss, mask_logits,) + outputs
        else:
            mask_logits = self.decoder(self.transform(sequence_output))
